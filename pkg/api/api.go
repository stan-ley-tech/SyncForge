...
//
// DeviceID is generated by the client, not the server: a device's identity
// (and therefore its version vector component) has to exist the moment it
// starts making offline writes, which may be long before it ever reaches a
// server. Registration just associates a bearer token with that
// already-chosen id. Re-registering the same DeviceID rotates its token
// rather than failing, so a client that loses the registration response
// can safely retry.
